...
public class Ally_RainbowCat extends Shooter{
    ...
    static private int attackcycle = 50;
    static private int bulletSpeed = 30;
    private static int atkDelay = 9;
    ...
    void shoot() {
        if(lane > 0){
            levelWorld.addBullet(new Bullet_RainbowCat(mutableATK, posX, posY-120, lane-1, bulletSpeed, levelWorld));
        }
        if(lane < 4){
            levelWorld.addBullet(new Bullet_RainbowCat(mutableATK, posX, posY, lane+1, bulletSpeed, levelWorld));
        }
        levelWorld.addBullet(new Bullet_RainbowCat(mutableATK, posX, posY+120, lane, bulletSpeed, levelWorld));
    }
